...
'''
@Author: YouShumin
@Date: 2019-12-27 17:07:45
@LastEditTime : 2019-12-27 18:03:40
@LastEditors  : YouShumin
@Description: 
@FilePath: /cmdb/utils/rabbitmq_rdapter.py
'''
...
class RabbitMQConnecError(RabbitMQError):
    pass
...
class _AstncConnection(object):
    ...
    def __init__(self, rabbitmq_url, io_loop, timeout=5):
        self._parameter = URLParameters(rabbitmq_url)
    ...
    @property
    def status_ok(self):
        return self._current_status != self.CLOSE_STATUS and self._current_status != self.TIMEOUT_STATUS
    ...
    def _connect(self):
        try:
            ...
            self.logger.error("fail to connect rabbitmq. %s", e)
    ...
    def _on_timeout(self, future):
        if self._current_status == self.CONNECTING_STATUS:
            self.logger.error("create connection time out")
    ...
    def _try_connect(self):
        ...
        def close_callback(connection, reply_code, reply_text):
            self.logger.error(
                "closint connection: reply code: %s, reply_text: %s. system will exist",
...
class TornadoAdapter(object):
    ...
    def __init__(self, rabbitmq_url, io_loop=None):
        ...
        self._io_loop = io_loop
        self._logger = logging.getLogger(__name__)
        self._publish_conn = _AstncConnection(rabbitmq_url, io_loop)
        self._receive_conn = _AstncConnection(rabbitmq_url, io_loop)
    ...
    def connect(self):
        warnings.warn(
            "Calling connect() method is unnecessary. "
    ...
    def _create_channel(self,
                        ...
                        close_callback=None):
        self.logger.info("cretae channel")
        future = Future()

        def on_channel_flow(*arfs, **kwargs):
            pass
        ...
        def on_channel_closed(reply_code, reply_txt):
            if reply_code not in [
                    self._NORMAL_CLOSE_CODE, self._USER_CLOSE_CODE
            ]:
                self.logger.error(
                    "channel closed. reply code: %d; reply text: %s. system will exist",
                ...
                if close_callback is not None:
                    ...
                    raise Exception(
                        "channel was close unexpected. reply code: %d, reply text: %s"
                        % (
                            reply_code,
                            reply_txt,
                        ))
            else:
                self.logger.info("reply code %s, reply txt: %s", reply_code,
                                 reply_txt)

        def on_channel_return(channel, method, property, body):
            self.logger.error(
                "reject from server. reply code: %d, reply text: %s.",
    ...
    def publish(self,
        ...
        try:
            ...
            self.logger.info(
                "preparing to publish. exchange: %s; routing_key: %s",
                exchange, routing_key)
            channel = yield self._create_channel(
                conn,
    ...
    def receive(self,
        ...
        try:
            ...
            self.logger.info(
                "[receive] exchange: %s; routing key: %s; queue name: %s",
                exchange, routing_key, queue_name)
            channel = yield self._create_channel(
                conn,
                close_callback=close_callback,
                cancel_callback=cancel_callback)
            yield self._queue_declare(channel,
                                      queue=queue_name,
                                      auto_delete=False,
                                      durable=True)
            if routing_key != "":
                yield self._exchange_declare(channel, exchange=exchange)
                yield self._queue_bind(channel,
                                       exchange=exchange,
                                       queue=queue_name,
                                       routing_key=routing_key)
            self.logger.info(
                "[start consuming] exchange: %s; routing key: %s; queue name: %s",
                exchange, routing_key, queue_name)
            channel.basic_qos(prefetch_count=prefetch_count)
            channel.basic_consume(functools.partial(
                self._on_message,
                exchange=exchange,
                handler=handler,
                return_callback=return_callback,
    ...
    def rpc(self,
        ...
        try:
            conn = yield self._receive_conn.get_connection()
            self.logger.info(
                "preparing to rpc call. exchange: %s; routing key: %s",
                exchange, routing_key)
            if exchange not in self._rpc_exchange_dict:
                self._rpc_exchange_dict[exchange] = Queue(maxsize=1)
                callback_queue = yield self._initialize_rpc_callback(
                    exchange,
                    conn,
                    cancel_callback=cancel_callback,
                    close_callback=close_callback)
            ...
            self.logger.info("starting rpc calling correlation id: %s",
                             corr_id)
            if corr_id in self._rpc_corr_id_dict:
                self.logger.warning("correlation id exists before calling. %s",
                                    corr_id)
    ...
    @gen.coroutine
    def _initialize_rpc_callback(self,
                                 exchange,
                                 conn,
                                 close_callback=None,
                                 cancel_callback=None):
        self.logger.info("initialize rpc callback queue")
        rpc_channel = yield self._create_channel(
            conn,
            close_callback=close_callback,
            cancel_callback=cancel_callback)
        callback_queue = yield self._queue_declare(rpc_channel,
                                                   auto_delete=True)
        self.logger.info("callback queue: %s", callback_queue)
        if exchange != "":
            yield self._exchange_declare(rpc_channel, exchange)
            yield self._queue_bind(rpc_channel,
                                   exchange=exchange,
    ...
    def _rpc_callback_process(self, unused_channel, basic_deliver, properties,
                              body):
        self.logger.info("rpc get response, correlation id: %s",
        ...
        if properties.correlation_id in self._rpc_corr_id_dict:
            ...
            self.logger.warning(
                "rpc get non exist response. correlation id: %s",
                properties.correlation_id)
        unused_channel.basic_ack(basic_deliver.delivery_tag)
        ...
        def on_timeout():
            if corr_id in self._rpc_corr_id_dict:
                ...
                future.set_exception(
                    RabbitMQTimeoutError('rpc timeout. corr id: %s' % corr_id))

        if timeout is not None:
            self._io_loop.add_timeout(datetime.timedelta(seconds=timeout),
                                      on_timeout)
        return future
